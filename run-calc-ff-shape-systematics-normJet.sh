...
outputLR=${sysDir}/jssys_longrange_${sample}_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs_final.root
outPutTmp="longrange_${sample}_${phoetmin}_${jetptmin}_gxi${gammaxi}_defnFF1_ff_final.root"
cp $outputLR $outPutTmp
varFileLR=${sysDir}/jssys_longrange_${sample}_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs_final.root
...
./calc_iso_systematics.exe $inputNominalIso $inputIso $nomFile $mcsample $sample $type $phoetmin $jetptmin $gammaxi jssys_iso $type $type hjs_normJet_final
...
touch $HISTLIST
echo -e "hjs_normJet_final_${sample}_${type}_100_200" >> $HISTLIST
if [ $sample = "pbpbdata" ] || ([ $sample = "ppdata" ] && ([ $type = "srecoreco" ] || [ $type = "scorrjsrecoreco" ])); then
  echo -e "hjs_normJet_final_${sample}_${type}_60_100" >> $HISTLIST
  echo -e "hjs_normJet_final_${sample}_${type}_20_60" >> $HISTLIST
  echo -e "hjs_normJet_final_${sample}_${type}_0_20" >> $HISTLIST
  echo -e "hjs_normJet_final_${sample}_${type}_60_200" >> $HISTLIST
  echo -e "hjs_normJet_final_${sample}_${type}_0_60" >> $HISTLIST
fi
...
mv sys_hjs_normJet_final_${sample}_${type}_*_*-data_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs.png ${sysDir}
...
cp ${sysDir}"/"jssys_data_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs-systematics.root .
./calc_ratio_systematics.exe js_normJet $SYSFILELIST $SYSHISTLIST jssys_data_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs
mv sys_hjs_normJet_final_*_*_*-jssys_data_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs.png ${sysDir}
mv jssys_data_${phoetmin}_${jetptmin}_gxi${gammaxi}_obs${obs}_ffjs-systematics.root ${sysDir}
