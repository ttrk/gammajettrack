...
void th1_set_bin_errors4diff_partialCorr(TH1D* hRatio, TH1D* hNum, TH1D* hDenom) {
    // reference : https://www.phenix.bnl.gov/WWW/publish/elke/EIC/Files-for-Wiki/lara.02-008.errors.pdf
    for (int i=1; i<=hRatio->GetNbinsX(); ++i) {
...
void sys_var_t::fit_sys(std::string diff_fit_func, std::string ratio_fit_func, double range_low, double range_high) {
    ...
    hratio_fit_diff = (TH1D*)hnominal->Clone(Form("%s_ratio_fit_diff", hist_name.c_str()));
...
void total_sys_var_t::add_sqrt_sum_squares_fit(TH1D* herr) {
    if (hsystematics_fit == 0) return;
    if (herr == 0) return;
...
void total_sys_var_t::add_sys_var(sys_var_t* sys_var, int option, int method) {
    switch (option) {
        ...
        case 1: {
            ...
            htmp = (TH1D*)sys_var->hratio_fit_diff->Clone("htmp");
            htmp->Multiply(hnominal);
