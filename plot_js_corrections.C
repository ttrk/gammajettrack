                    ...
                    for (int iTrkPt = 0; iTrkPt < nTrkPtBins*2 + 1; ++iTrkPt) {
                        for (int i = 0; i < nSteps; ++i) {
                            ...
                            for (int iCent = 0; iCent < nCentBins; ++iCent) {
                                ...
                                histCorrName = Form("%s_corrsmooth_%s_%s2%s_%sptBin%d_etaBin%d%s_%d_%d", stepsDenomPrefixes[i].c_str(),
                                        sample.c_str(), recoGenStepsDenom[i].c_str(), recoGenStepsNum[i].c_str(),
                                        ptTypes[iPtType].c_str(), iPt, iEta, strTrkPt.c_str(), min_hiBin[iCent], max_hiBin[iCent]);
                                cnvName = Form("cnv_%s_corrsmooth_%s_%s2%s_%sptBin%d_etaBin%d%s", stepsDenomPrefixes[i].c_str(),
                                        sample.c_str(), recoGenStepsDenom[i].c_str(), recoGenStepsNum[i].c_str(),
                                        ptTypes[iPtType].c_str(), iPt, iEta, strTrkPt.c_str());
                                ...
                                std::string stepStr = Form("Correction Step %d", i+1);
                                std::string ptStr = Form("%d < p_{T}^{jet} < %d GeV/c", ptBins[iPt], ptBins[iPt+1]);
                                if (ptBins[iPt+1] >= 9999) {
                                    ptStr = Form("p_{T}^{jet} > %d GeV/c", ptBins[iPt]);
                                }
                                std::string etaStr = Form("%.1f < |#eta^{jet}| < %.1f", etaBins[iEta], etaBins[iEta+1]);
                    ...
                    for (int iTrkPt = 0; iTrkPt < nTrkPtBins*2 + 1; ++iTrkPt) {
                        for (int i = 0; i < nSteps; ++i) {
                            ...
                            for (int iCent = 0; iCent < nCentBins; ++iCent) {
                                ...
                                histCorrName = Form("%s_corrsmoothv3_%s_%s2%s_%sptBin%d_etaBin%d%s_%d_%d", stepsDenomPrefixes[i].c_str(),
                                        sample.c_str(), recoGenStepsDenom[i].c_str(), recoGenStepsNum[i].c_str(),
                                        ptTypes[iPtType].c_str(), iPt, iEta, strTrkPt.c_str(), min_hiBin[iCent], max_hiBin[iCent]);
                                cnvName = Form("cnv_%s_corrsmoothv3_%s_%s2%s_%sptBin%d_etaBin%d%s", stepsDenomPrefixes[i].c_str(),
                                        sample.c_str(), recoGenStepsDenom[i].c_str(), recoGenStepsNum[i].c_str(),
                                        ptTypes[iPtType].c_str(), iPt, iEta, strTrkPt.c_str());
                    ...
                    for (int iTrkPt = 0; iTrkPt < nTrkPtBins*2 + 1; ++iTrkPt) {
                        for (int i = 0; i < nSteps; ++i) {
                            ...
                            for (int iCent = 0; iCent < nCentBins; ++iCent) {
                                ...
                                histCorrName = Form("%s_diff_%s_%s2%s_%sptBin%d_etaBin%d%s_%d_%d", stepsDenomPrefixes[i].c_str(),
                                        sample.c_str(), recoGenStepsDenom[i].c_str(), recoGenStepsNum[i].c_str(),
                                        ptTypes[iPtType].c_str(), iPt, iEta, strTrkPt.c_str(), min_hiBin[iCent], max_hiBin[iCent]);
                                cnvName = Form("cnv_%s_diff_%s_%s2%s_%sptBin%d_etaBin%d%s", stepsDenomPrefixes[i].c_str(),
                                        sample.c_str(), recoGenStepsDenom[i].c_str(), recoGenStepsNum[i].c_str(),
                                        ptTypes[iPtType].c_str(), iPt, iEta, strTrkPt.c_str());
