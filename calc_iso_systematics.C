...
int calc_iso_systematics(const char* nominal, const char* variation, const char* data, const char* sample, const char* output_sample, const char* type, int phoetmin, int jetptmin, int gammaxi) {
    ...
    for (int i=0; i<ncent; ++i) {
        hnominal[i] = (TH1D*)fnominal->Get(Form("%s_%s_%s_%s", histPrefix, sample, type, centsuffix[i]))->Clone(Form("hnominal_%s", centsuffix[i]));
        hvariation[i] = (TH1D*)fvariation->Get(Form("%s_%s_%s_%s", histPrefix, sample, type, centsuffix[i]))->Clone(Form("hvariation_%s", centsuffix[i]));
